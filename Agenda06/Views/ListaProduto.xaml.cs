...
public partial class ListaProduto : ContentPage
{
    ...
    private void AplicarFiltroCategoria()
    {
        ...
        string categoria = cmb_categoria.SelectedItem as string;

        IEnumerable<Produto> filtrados = listaCompleta;

        if (!string.IsNullOrEmpty(categoria) && categoria != "Todas")
        {
            filtrados = listaCompleta.Where(p => p.Categoria == categoria);
        }

        foreach (Produto p in filtrados)
        {
            lista.Add(p);
        }
    }
